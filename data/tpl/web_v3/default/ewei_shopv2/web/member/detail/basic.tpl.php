...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
        <select name='data[fid]' class='form-control select2'  multiple data-placeholder="选择推荐人">
            <?php  if(is_array($list)) { foreach($list as $lists) { ?>
            <option value='<?php  echo $lists['id'];?>' <?php  if($lists['id']== $member['fid'] ) { ?>selected<?php  } else { ?><?php  } ?>><?php  echo $lists['mobile'];?></option>
            <?php  } } ?>
        </select>
    </div>
</div>
<div class="form-group">
    <label class="col-lg control-label">佣金</label>
    <div class="col-sm-3">
        <div class='form-control-static'><?php  echo $member['brokerage'];?>
            <?php if(cv('finance.recharge.credit2')) { ?>
                <a class="text-primary " data-toggle='ajaxModal' href="<?php  echo webUrl('finance/recharge', array('type'=>'brokerage','id'=>$member['id']))?>" style="padding-left: 5px;">充值</a>
            <?php  } ?>
        </div>
    </div>
</div>
...
<div class="form-group">
    <label class="col-lg control-label">下级人数</label>
    <div class="col-sm-3">
        <div class='form-control-static'><?php  echo $member['invite'];?>
        </div>
    </div>
</div>

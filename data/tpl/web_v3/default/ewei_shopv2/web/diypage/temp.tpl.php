...
<div class="page-content">
	...
	<div class="form-horizontal">
		<div class="form-group">
			...
			<div class="col-sm-10">
				<a class="btn btn-xs <?php  if($_GPC['type']=='') { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>'', 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">全部类型</a>
				<a class="btn btn-xs <?php  if($_GPC['type']==1) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>1, 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">自定义页面</a>
				<a class="btn btn-xs <?php  if($_GPC['type']==2) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>2, 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">商城首页</a>
				<?php  if($_W['plugin']!='merch'&&empty($_W['merchid'])) { ?>
					<a class="btn btn-xs <?php  if($_GPC['type']==3) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>3, 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">会员中心</a>
					<?php  if(p('commission')) { ?>
					<a class="btn btn-xs <?php  if($_GPC['type']==4) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>4, 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">分销中心</a>
					<?php  } ?>
					<a class="btn btn-xs <?php  if($_GPC['type']==5) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>5, 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">商品详情</a>
					<?php  if(p('creditshop')) { ?>
					<a class="btn btn-xs <?php  if($_GPC['type']==6) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>6, 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">积分商城</a>
					<?php  } ?>
					<?php  if(p('seckill')) { ?>
					<a class="btn btn-xs <?php  if($_GPC['type']==7) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>7, 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">整点秒杀</a>
					<?php  } ?>
					<?php  if(p('exchange')) { ?>
					<a class="btn btn-xs <?php  if($_GPC['type']==8) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('type'=>8, 'cate'=>$_GPC['cate'], 'keyword'=>$_GPC['keyword']))?>">兑换中心</a>
					<?php  } ?>
				<?php  } ?>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-10">
				<a class="btn btn-xs <?php  if($_GPC['cate']=='') { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('cate'=>'', 'type'=>$_GPC['type'], 'keyword'=>$_GPC['keyword']))?>">全部分类</a>
				<a class="btn btn-xs <?php  if($_GPC['cate']=='0') { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('cate'=>'0', 'type'=>$_GPC['type'], 'keyword'=>$_GPC['keyword']))?>">未分类</a>
				<?php  if(is_array($category)) { foreach($category as $item) { ?>
				<a class="btn btn-xs <?php  if($_GPC['cate']==$item['id']) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo webUrl('diypage/temp', array('cate'=>$item['id'], 'type'=>$_GPC['type'], 'keyword'=>$_GPC['keyword']))?>"><?php  echo $item['name'];?></a>
				<?php  } } ?>
			</div>
		</div>
	</div>

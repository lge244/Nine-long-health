...
<div class="page-header">当前位置：<span class="text-primary">商城页面设置</span></div>
...
<div class="page-content">
    ...
    <form action="" <?php if(cv('diypage.shop.page.save')) { ?>method="post"<?php  } ?> class="form-horizontal form-validate">
        ...
        <div class="form-group">
            <label class="col-lg control-label">商城首页</label>
            <div class="col-sm-9 col-xs-12">
                <select class="form-control valid select2" <?php if(cv('diypage.shop.page.save')) { ?>name="page[home]"<?php  } else { ?> disabled<?php  } ?>>
        ...
        <?php  if($_W['plugin']!='merch' && !isset($_W['merch'])) { ?>
            <div class="form-group">
                <label class="col-lg control-label">会员中心</label>
                <div class="col-sm-9 col-xs-12">
                    <select class="form-control valid select2" <?php if(cv('diypage.shop.page.save')) { ?>name="page[member]"<?php  } else { ?> disabled<?php  } ?>>
                    <option value="">系统默认</option>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9 col-xs-12">
                    <select class="form-control valid select2" <?php if(cv('diypage.shop.page.save')) { ?>name="page[detail]"<?php  } else { ?> disabled<?php  } ?>>
                    <option value="">系统默认</option>
        ...
        <?php  if($_W['plugin']!='merch' && !isset($_W['merch'])) { ?>
            ...
            <?php  if(is_array($pluginList)) { foreach($pluginList as $pluginname => $plugin) { ?>
                <?php  if(!empty($plugin['status']) && p($pluginname)) { ?>
                    <div class="form-group">
                        <label class="col-lg control-label"><?php  echo m('plugin')->getName($pluginname)?></label>
                        <div class="col-sm-9 col-xs-12">
                            <select class="form-control valid select2" <?php if(cv('diypage.shop.page.save')) { ?>name="page[<?php  echo $pluginname;?>]"<?php  } else { ?> disabled<?php  } ?>>
                            <option value="">系统默认</option>

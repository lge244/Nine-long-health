...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		{
			if( isset($set["pay"]) && $set["pay"]["alipay"] == 1 && is_array($setting["payment"]["alipay"]) && ($setting["payment"]["alipay"]["switch"] || $setting["payment"]["alipay"]["pay_switch"]) ) 
			{
				$params = array( );
				$params["tid"] = trim($log["tid"]);
	...
	public function complete() 
	{
		...
		if( !empty($order["istrade"]) && (1 < $order["status"] || $order["status"] == 1 && $order["tradestatus"] == 2) ) 
		{
			$go_flag = 1;
		}
		if( $go_flag == 1 ) 
		...
		if( (empty($order["isnewstore"]) || empty($order["storeid"])) && empty($order["istrade"]) ) 
		{
			...
			foreach( $order_goods as $data ) 
			{
				...
				if( $data["seckill"] && (empty($seckillinfo) || $seckillinfo["status"] != 0 || $seckillinfo["endtime"] < time()) ) 
				{
					if( $_W["ispost"] ) 
					{

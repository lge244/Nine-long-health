...
class Picker_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		else if (($goods['hasoption'] == 1) && ($goods['hascommission'] == 0) && ($goods['nocommission'] == 0)) 
		{
			...
			foreach ($gifts as $key => &$value ) 
			{
				...
				foreach ($gid as $ke => $val ) 
				{
					if ($val == $id) 
					{
						...
						{
							$isgift = 1;
							$gifts[$key]['gift'][$k] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('ewei_shop_goods') . ' where uniacid = ' . $_W['uniacid'] . ' and deleted = 0 and total > 0 and status = 2 and id = ' . $val . ' ');
						}
					}
				}
				if (empty($value['gift'])) 

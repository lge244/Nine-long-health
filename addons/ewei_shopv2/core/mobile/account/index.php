...
class Index_EweiShopV2Page extends MobilePage
{
 ...
 $uid =  $_GPC['uid'];
		...
		if ($_W['ispost']) {
			...
			if (empty($mobile)) {
                exit(json_encode(array('code'=>1,'msg'=>"请输入正确的手机号")));
            }
			...
			if (empty($type)) {
				if (!empty($member)) {
                    exit(json_encode(array('code'=>1,'msg'=>"此手机号已注册, 请直接登录")));
                }
				...
				@$data = array('uniacid' => $_W['uniacid'],'fid'=>(int)$uid, 'mobile' => $mobile, 'nickname' => $nickname, 'openid' => $openid, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'createtime' => time(), 'mobileverify' => 1, 'comefrom' => 'mobile');
			}
			else {
				if (empty($member)) {
                    show_json(0, '此手机号未注册');
...
//				pdo_update('ewei_shop_member',array('invite'=> $invite['invite']),array('id'=>$uid));
			...
			if (p('commission')) {
                ...
                unset($_SESSION[$key]);
                exit(json_encode(array('code'=>0,'msg'=>empty($type) ? '注册成功' : '密码重置成功')));

...
class Verifyorder_EweiShopV2Page extends MobilePage
{
    public function orderData()
    {
        ...
        {
            $sql = "SELECT o.*,0 as isverifygoods_order FROM" . tablename("ewei_shop_order") . " o inner JOIN " . tablename("ewei_shop_saler") . " s ON o.verifyopenid=s.openid   WHERE s.openid=:openid AND o.ismr=0  AND   o.status =3  AND o.deleted=0 and o.uniacid=:uniacid   GROUP BY o.id ORDER BY o.createtime DESC  ";
            if( empty($_GPC["export"]) ) 
            {
                $sql .= "LIMIT " . ($pindex - 1) * $psize . "," . $psize;
            }
        ...
        $paytype = array( array( "css" => "default", "name" => "未支付" ), array( "css" => "danger", "name" => "余额支付" ), 11 => array( "css" => "default", "name" => "后台付款" ), 2 => array( "css" => "danger", "name" => "在线支付" ), 21 => array( "css" => "success", "name" => "微信支付" ), 22 => array( "css" => "warning", "name" => "支付宝支付" ), 23 => array( "css" => "warning", "name" => "银联支付" ), 3 => array( "css" => "primary", "name" => "货到付款" ) );
        ...
        if( !empty($list) ) 
        {
            ...
            foreach( $list as &$value ) 
            {
                ...
                $m2 = false;
                $m3 = false;
                if( !empty($level) && empty($agentid) && !empty($value["agentid"]) ) 
                {
                ...
                foreach( $order_goods as &$og ) 
                {
                    ...
                    if( p("diyform") && !empty($og["diyformfields"]) && !empty($og["diyformdata"]) ) 
                    {
                        ...
                        {
                            if( !empty($diy_title_data) && array_key_exists($da["key"], $diy_title_data) ) 
                            {
                                $dflag = 0;
                            }

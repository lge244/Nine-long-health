...
class Page extends WeModuleSite
{
    public function runTasks()
    {
        ...
        if (isset($_W['ewei_shopv2_member'])) {
            $this->checkLevel();
        }
        if (empty($interval)) {
            $interval = 60;
 ...
 public function checkLevel()
    {
        ...
        // 获取当前用户登录的openid
        $openid = $_W['openid'];
        // 根据openid 来获取当前用户完成的订单
   
     $order= pdo_getall('ewei_shop_order', array( "openid" => $_W['openid'],"status"=>3), array('id'));
        $order_Num = 0;
        foreach ($order as $k=>$v){
            $orderNum = pdo_get('ewei_shop_order_goods',array('orderid'=>$v['id']),array('total'));
            $order_Num +=$orderNum['total'];
        }   
  
        // 获取等级的要求
        $level = pdo_getall('ewei_shop_member_level', array(), array('id', 'ordermoney', 'ordercount', 'recommend', 'goodsid','attend'));
        // 获取当前登录的用户信息
        $My_member = pdo_getall('ewei_shop_member', array('fid' => $_W['ewei_shopv2_member']['id']), array('id', 'level', 'fid', 'invite'));
        $invite = (int)$_W['ewei_shopv2_member']['invite'];
 		array_shift($level);
        // 获取用户的上级
        $fid = $_W['ewei_shopv2_member']['id'];

        $curriculum = count(pdo_getall('ewei_shop_curriculum',array('status'=>1,'openid'=>$_W['openid'])));

        // 特殊通道 起始完成 4调订单直接升级到5级
        if ($order_Num >= 4 && $_W['ewei_shopv2_member']['level'] == 0) {
            pdo_update('ewei_shop_member', array('level' => 5), array('id' => $_W['ewei_shopv2_member']['id']));
        }
        $level_lng = count($level);
        for ($i=0; $i<= $level_lng-1; $i++){
            if ($level[$i]['goodsid'] > 0){
                // 查询用户是否购买必须购买的商品
                $goodsid = pdo_get('ewei_shop_order_goods', array('goodsid' => $level[$i]['goodsid'], 'openid' => $_W['ewei_shopv2_member']['openid']));
                $goodsid = pdo_get('ewei_shop_order',array('id'=>$goodsid['orderid'],'status'=>3));
                if ($invite >= $level[$i]['recommend'] && $invite < $level[$i+1]['recommend'] && $order_Num >= $level[$i]['ordercount'] && $order_Num >= $level[$i+1]['ordercount'] && $curriculum >= $level[$i]['attend'] && $curriculum < $level[$i+1]['attend'] && $goodsid > 0) {
                    pdo_update('ewei_shop_member', array('level' => $level[$i]['id']), array('id' => $_W['ewei_shopv2_member']['id']));
                    exit();
                }
            } else{
                if ($invite >= $level[$i]['recommend'] && $invite <= $level[$i+1]['recommend'] && $order_Num >= $level[$i]['ordercount'] && $order_Num <= $level[$i+1]['ordercount'] && $curriculum >= $level[$i]['attend'] && $curriculum <= $level[$i+1]['attend']) {
                    pdo_update('ewei_shop_member', array('level' => $level[$i]['id']), array('id' => $_W['ewei_shopv2_member']['id']));
                    exit();
                }
            }
    ...
    // 递归得到用户邀请的总人数
//    public function recursionCount($array)
//    {
...
//            if ($value['invite'] > 0) {
//                $array2 = pdo_getall('ewei_shop_member', array('fid' => $value['id']), array('id', 'level', 'fid', 'invite'));
//                foreach ($array2 as $k => $v) {
//                    if ($v['invite'] > 0) {
//                        $array3 = pdo_getall('ewei_shop_member', array('fid' => $v['id']), array('id', 'level', 'fid', 'invite'));
//                        $this->recursionCount($array3);
//                    }
//                    $down_num++;
//                }
//            }
...
//    {
//        $member = pdo_get('ewei_shop_member', array('id' => $fid), array('id', 'level', 'fid', 'invite'));
//        if ($member['level'] >= 5) {
//            $f_member = pdo_get('ewei_shop_member', array('id' => $member['id']));
//        } else {
//            if ($member['fid'] > 0) {
////          查找上级
//                $this->acquire($member['fid']);
//            }
//        }
//        return $f_member;
//    }

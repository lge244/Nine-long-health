...
class Printer_EweiShopV2ComModel extends ComModel
{
    ...
    public function printRechargeMessage($params, $printerid)
    {
        ...
        $this->template = array( "print_title" => "会员充值", "print_style" => ":10|:22", "print_data" => array( "key" => array( "会员昵称：|" . $params["nickname"], "充值金额：|￥" . $params["price"], "支付方式：|" . $params["paytype"], "充值时间：|" . $params["paytime"] ), "value" => array( "0", "0", "0" ) ) );
        $this->printer = $printer;
        load()->func("communication");
        switch( $printer["type"] ) 
        {
    ...
    public function sendOrderMessage($orderid = 0, $verify = array(  ))
    {
        ...
        $PrinterSet = $this->getPrinterSet();
        $res = array(  );
        if( (!empty($PrinterSet["printer_merch"]) || empty($PrinterSet["printer_merch"]) && $order["merchid"] == 0) && !empty($PrinterSet["ordertype"]) && !empty($PrinterSet["order_printer"]) && in_array($status, $PrinterSet["ordertype"]) ) 
        {
            foreach( $PrinterSet["order_printer"] as $value ) 
            {
        ...
        if( 0 < $order["merchid"] ) 
        {
            ...
            if( !empty($PrinterSet["ordertype"]) && !empty($PrinterSet["order_printer"]) && in_array($status, $PrinterSet["ordertype"]) ) 
            {
                foreach( $PrinterSet["order_printer"] as $value ) 
                {
                    $res[] = $this->printer($params, $PrinterSet["order_template"], $value["id"], 0);
        ...
        if( isset($StorePrinterSet) && !empty($StorePrinterSet["ordertype"]) && !empty($StorePrinterSet["order_printer"]) && in_array($status, $StorePrinterSet["ordertype"]) ) 
        {

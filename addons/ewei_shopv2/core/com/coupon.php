...
class Coupon_EweiShopV2ComModel extends ComModel 
{
	...
	public function sendBackMessage($openid, $coupon, $gives) 
	{
		...
		$content .= $givestr;
		$content .= '，请查看您的账户，谢谢!';
		$msg = array( 'keyword1' => array('value' => '优惠券返利', 'color' => '#73a68d'), 'keyword2' => array('value' => $content, 'color' => '#73a68d'), 'keyword3' => array('value' => date('Y-m-d H:i:s'), 'color' => '#000000') );
		$url = mobileUrl('member', NULL, true);
		if (!(empty($templateid))) 
		{
	...
	public function sendReturnMessage($openid, $coupon) 
	{
		...
		$set = m('common')->getPluginset('coupon');
		$templateid = $set['templateid'];
		$msg = array( 'keyword1' => array('value' => '优惠券退回', 'color' => '#73a68d'), 'keyword2' => array('value' => '您的优惠券【' . $coupon['couponname'] . '】已退回您的账户，您可以再次使用, 谢谢!', 'color' => '#73a68d'), 'keyword3' => array('value' => date('Y-m-d H:i:s'), 'color' => '#000000') );

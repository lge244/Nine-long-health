...
class Printer_EweiShopV2ComModel extends ComModel 
{
	...
	public function style_list_code($str, $code = 0, $type = 0) 
	{
		...
		else if ($type == '6') 
		{
			switch ($code) 
			{
				...
				case self::CONTENT_CENTER: $res = '<center>' . trim($str) . '</center>';
				break;
				case self::CONTENT_CENTER_BOLD: $res = '<FB><center>' . trim($str) . '</center></FB>' . "\n";
				break;
				case self::CONTENT_CODE: $res = '<BR>' . $str . '</BR>';
				break;
				case self::CONTENT_QRCODE: $res = '<QR>' . $str . '</QR>';
	...
	public function printer($params, $templateid, $printerid, $type = 0) 
	{
		...
		switch ($printer['type']) 
		{
			...
			break;
			case self::PRINTRT_YILIANYUN_AUTH2: $res = $this->printerYilianyunAuth2($printer['merchid']);
			break;
	...
	public function cut_str($sourcestr, $cutlength) 
	{
		...
		while (strlen($sourcestr) && ($i <= $str_length)) 
		{

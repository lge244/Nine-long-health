...
class Member_EweiShopV2Model 
{
	...
	public function upgradeLevel($openid, $orderid = 0) 
	{
		...
		if (empty($leveltype)) 
		{
            ...
            $invitecount = pdo_fetchcolumn('select  invite from ' . tablename('ewei_shop_member') . ' where id=:agentid and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['mid']));
            $level = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . ' where uniacid=:uniacid  and enabled=1 and '.$invitecount.'>=recommend and recommend>0  and '. $ordermoney . ' >= ordermoney and ordermoney>0  order by level desc limit 1', array(':uniacid' => $_W['uniacid']));
		}
		else if ($leveltype == 1) 
		{
            $ordermoney = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid ' . ' where o.openid=:openid and o.status=3 and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));

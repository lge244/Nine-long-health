	...
	public function main() 
	{
		...
		if ($_GPC['isblack'] != '') 
		{
			$condition .= ' and dm.isblack=' . intval($_GPC['isblack']);
              ...
              $condition .=' and dm.mobile != \'\'';
		$sql = 'select * ,dm.openid from ' . tablename('ewei_shop_member') . ' dm '  . ' where 1 ' . $condition . '  ORDER BY dm.id DESC,dm.createtime DESC ';
		...
		foreach ($list as &$row ) 
		{
			...
			$row['agentnickname'] = ((isset($res_agent[$row['fid']]) ? $res_agent[$row['fid']]['agentnickname'] : ''));
			$row['agentavatar'] = ((isset($res_agent[$row['fid']]) ? $res_agent[$row['fid']]['agentavatar'] : ''));
					...
					if (!(empty($adata))) 
					{
						...
						if ($adata['agentid'] != $member['agentid']) 
						{
							if (p('dividend')) 
							{
								...
								{
									pdo_update('ewei_shop_member', array('headsid' => 0), array('headsid' => $member['headsid'], 'uniacid' => $_W['uniacid']));
									p('commission')->delRelation($member['id']);
									p('commission')->saveRelation($member['id'], $member['headsid'], 1);
									$dividend = pdo_fetch('select id,isheads,headsid,headsstatus from ' . tablename('ewei_shop_member') . ' where id = :id', array(':id' => $member['headsid']));
									$dividend_init = pdo_fetch('select * from ' . tablename('ewei_shop_dividend_init') . ' where headsid = :headsid', array(':headsid' => $member['headsid']));
									if (!(empty($dividend['isheads'])) && !(empty($dividend['headsstatus'])) && !(empty($dividend_init['status']))) 
									{
										$data = pdo_fetchall('select id from ' . tablename('ewei_shop_commission_relation') . ' where pid = :pid', array(':pid' => $member['headsid']));
										if (!(empty($data))) 
										{
											$ids = array();
											foreach ($data as $k => $v ) 
											...
											pdo_update('ewei_shop_member', array('headsid' => $member['headsid']), array('id' => $ids));
										}
									}
								}
								else 
								{
									...
									if (!(empty($dividend['isheads'])) && !(empty($dividend['headsstatus'])) && !(empty($dividend_init['status']))) 
									{
										$data = pdo_fetchall('select id from ' . tablename('ewei_shop_commission_relation') . ' where pid = :pid', array(':pid' => $adata['agentid']));
										if (!(empty($data))) 
										{
											$ids = array();
											foreach ($data as $k => $v ) 

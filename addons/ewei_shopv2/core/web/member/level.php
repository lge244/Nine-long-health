...
class Level_EweiShopV2Page extends WebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			$data = array('uniacid' => $_W['uniacid'], 'level' => intval($_GPC['level']), 'levelname' => trim($_GPC['levelname']), 'ordercount' => intval($_GPC['ordernum']), 'ordermoney' => $_GPC['ordermoney'], 'discount' => trim($_GPC['discount']), 'enabled' => $enabled,"recommend"=>$_GPC['recommend'],'goodsid'=>$_GPC['product'],'attend'=>$_GPC['attend']);
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					$a= pdo_update('ewei_shop_member_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));

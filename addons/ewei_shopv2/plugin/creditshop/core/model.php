	...
	class CreditshopModel extends PluginModel 
	{
		...
		public function packetmoney($goodsid) 
		{
			...
			if ($goods['packettype'] == 1) 
			{
				...
				else if ($goods['maxpacketmoney'] < $MoneyPackage['remainMoney']) 
				{
			...
			if ($log['status'] == 2) 
			{
				if (!(empty($type))) 
				{
					if ($log['status'] == 2) 
					{
						...
						$msg = array( 'first' => array('value' => '恭喜您，您中奖啦~', 'color' => '#4a5077'), 'keyword1' => array('title' => '活动', 'value' => '【' . $shop['name'] . '】抽奖', 'color' => '#4a5077'), 'keyword2' => array('title' => '奖品', 'value' => $goods['title'], 'color' => '#4a5077'), 'keyword3' => array('title' => '时间', 'value' => date('Y-m-d H:i', time()), 'color' => '#4a5077'), 'remark' => array('value' => $remark, 'color' => '#4a5077') );
						if (!(empty($tm['award']))) 
						{
				...
				if (($log['dispatchstatus'] == 1) || ($log['dispatchstatus'] == -1)) 
				{
					...
					$msg = array( 'first' => array('value' => '积分商城商品兑换成功~', 'color' => '#4a5077'), 'keyword1' => array('title' => '商品信息', 'value' => $goods['title'], 'color' => '#4a5077'), 'keyword2' => array('title' => '订单金额', 'value' => $credits, 'color' => '#4a5077'), 'keyword3' => array('title' => '兑换时间', 'value' => date('Y-m-d H:i', $log['createtime']), 'color' => '#4a5077'), 'keyword4' => array('title' => '兑换码', 'value' => ($goods['isverify'] ? '已发放' : '物流配送'), 'color' => '#4a5077'), 'remark' => array('value' => $remark, 'color' => '#4a5077') );

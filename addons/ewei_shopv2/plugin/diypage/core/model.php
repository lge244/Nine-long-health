                ...
                foreach( $page["data"]["items"] as $itemid => &$item ) 
                {
                    if( $item["id"] == "goods" ) 
                    {
                        ...
                        if( $item["params"]["goodsdata"] == "0" ) 
                        {
                            if( !empty($item["data"]) && is_array($item["data"]) ) 
                            {
                                ...
                                if( !empty($goodsids) && is_array($goodsids) ) 
                                {
                                    ...
                                    {
                                        $goods = pdo_fetchall("select id, title, subtitle, thumb, productprice, minprice, total,`type`,showlevels, showgroups,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice, bargain, merchid, sales, salesreal from " . tablename("ewei_shop_goods") . " where id in( " . $newgoodsids . " ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ", array( ":uniacid" => $_W["uniacid"] ));
                                    }
                        ...
                        else
                        {
                            ...
                            {
                                if( $item["params"]["goodsdata"] == "2" && empty($item["params"]["goodstype"]) && !empty($item["params"]["groupid"]) ) 
                                {
                            ...
                            else
                            {
                                if( $item["params"]["goodsdata"] == "2" && empty($item["params"]["goodstype"]) ) 
                                {
                                    ...
                                    if( !empty($group) && !empty($group["goodsids"]) ) 
                                    {
                                        ...
                                        $goods = pdo_fetchall("select id, title, subtitle, thumb, `type`, minprice, sales, salesreal, total, showlevels, showgroups,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,bargain, productprice from " . tablename("ewei_shop_goods") . " where id in( " . $goodsids . " ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid " . $orderby . " limit " . $limit, array( ":uniacid" => $_W["uniacid"] ));
                                        if( !empty($goods) && is_array($goods) ) 
                                        {
                                            $level = $this->getLevel($_W["openid"]);
                                            $set = m("common")->getPluginset("commission");
                                            ...
                                            else
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        {
                                                            if( $item["id"] == "commission_sharecode" ) 
                                                            {
                                                                                ...
                                                                                else
                                                                                {
                                                                                    if( $item["id"] == "tabbar" ) 
                                                                                    {
                                                                                        if( !empty($item["data"]) ) 
                                                                                        {
                                                                                            ...
                                                                                            {
                                                                                                $active = false;
                                                                                                if( !empty($child["linkurl"]) && strexists($_W["siteurl"], ltrim($child["linkurl"], "./")) ) 
                                                                                                {
...
, $str);
    ...
    {
        $pagetype = array( 1 => array( "name" => "自定义", "pagetype" => "diy", "class" => "" ), 2 => array( "name" => "商城首页", "pagetype" => "sys", "class" => "success" ), 3 => array( "name" => "会员中心", "pagetype" => "sys", "class" => "primary" ), 4 => array( "name" => "分销中心", "pagetype" => "plu", "class" => "warning" ), 5 => array( "name" => "商品详情页", "pagetype" => "sys", "class" => "danger" ), 6 => array( "name" => "积分商城", "pagetype" => "plu", "class" => "info" ), 7 => array( "name" => "整点秒杀", "pagetype" => "plu", "class" => "danger" ), 8 => array( "name" => "兑换中心", "pagetype" => "plu", "class" => "success" ), 9 => array( "name" => "快速购买", "pagetype" => "sys", "class" => "warning" ), 99 => array( "name" => "公用模块", "pagetype" => "mod", "class" => "" ) );
        if( !empty($type) ) 
        {
    ...
    public function calculate($str = NULL, $pagetype = false)
    {
        ...
        if( strexists($str, "r=globonus") ) 
        {
            ...
            if( $plugin_globonus ) 
            {
                ...
                {
                    if( $pagetype == 4 && !empty($plugin_globonus_set["closecommissioncenter"]) ) 
                    {
                        $plugins["globonus"] = false;
                    }
        ...
        if( strexists($str, "r=author") ) 
        {
            ...
            if( $plugin_author ) 
            {
                ...
                {
                    if( $pagetype == 4 && !empty($plugin_author_set["closecommissioncenter"]) ) 
                    {
                        $plugins["author"] = false;
                    }
...
, $str);
    ...
    public function detailPage($pageid = 0)
    {
        ...
        if( (empty($pageid) || !empty($pageid) && empty($page)) && !empty($set["page"]["detail"]) ) 
        {
        ...
        if( !empty($detail_navbar) ) 
        {
            ...
            if( intval($detail_navbar["params"]["hidelike"]) < 1 ) 
            {
                ...
                {
                    if( $detail_navbar["params"]["hidelike"] == -1 ) 
                    {
                        $navbar[] = $btnshop;
                    }
                    else
                    {
                        if( $detail_navbar["params"]["hidelike"] == -2 ) 
                        {
                            $navbar[] = $btncart;
                        }
                        else
            ...
            if( intval($detail_navbar["params"]["hideshop"]) < 1 ) 
            {
                ...
                {
                    if( $detail_navbar["params"]["hideshop"] == -1 ) 
                    {
                        $navbar[] = $btnlike;
                    }
                    else
                    {
                        if( $detail_navbar["params"]["hideshop"] == -2 ) 
                        {
                            $navbar[] = $btncart;
                        }
                        else
            ...
            if( intval($detail_navbar["params"]["hidecart"]) < 1 ) 
            {
                ...
                {
                    if( $detail_navbar["params"]["hidecart"] == -1 ) 
                    {
                        $navbar[] = $btnlike;
                    }
                    else
                    {
                        if( $detail_navbar["params"]["hidecart"] == -2 ) 
                        {
                            $navbar[] = $btnshop;
                        }
                        else
    ...
    public function seckillPage($pageid = 0)
    {
        ...
        if( (empty($pageid) || !empty($pageid) && empty($page)) && !empty($set["page"]["seckill"]) ) 
        {
        ...
        return array( "seckill_list" => $seckill_list, "diylayer" => $page["data"]["page"]["diylayer"], "diymenu" => $page["data"]["page"]["diymenu"], "diyadv" => $page["data"]["page"]["diyadv"], "items" => $pageitems, "background_color" => $page["data"]["page"]["background"], "seckill_style" => $page["data"]["page"]["seckill"]["style"], "seckill_color" => $page["data"]["page"]["seckill"]["color"] );
    }
    ...
    public function exchangePage($pageid = 0)
    {
        ...
        if( (empty($pageid) || !empty($pageid) && empty($page)) && !empty($set["page"]["exchange"]) ) 
        {

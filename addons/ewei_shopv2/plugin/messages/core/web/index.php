...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	public function build() 
	{
		...
		if (!(empty($item))) 
		{
			...
			$sql .= ' SELECT  :uniacid,sm.openid,sm.nickname,:taskid,1 from ' . tablename('ewei_shop_member') . ' sm  left join ' . tablename('mc_mapping_fans') . ' mf  on sm.openid=mf.openid and mf.follow=1 where sm.uniacid=:uniacid ';
	...
	public function sendTplNotice($openid, $templateid, $nickname) 
	{
		...
		if (!(empty($templateid))) 
		{
			...
			$msg = array( 'first' => array('value' => $template['first'], 'color' => $template['firstcolor']), 'remark' => array('value' => "\n" . $template['remark'], 'color' => $template['remarkcolor']) );
			$i = 0;
			while ($i < count($data)) 
			{

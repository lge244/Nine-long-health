...
class LiveModel extends PluginModel 
{
	...
	public function getLiveInfo($url = NULL, $type = "auto") 
	{
		...
		switch( $type ) 
		{
			...
			break;
			case "inke": $url = str_replace("share_uid", "share", $url);
			...
			$apiResult2 = ihttp_get($apiResult["hlsVideoUrl"] . "&clientType=1&jsoncallback=renren");
			if( empty($apiResult2["content"]) ) 
			{
				return error(2, "获取房间信息失败(video_content error)");
			}
			$apiResult2 = ltrim($apiResult2["content"], "renren(");
			$apiResult2 = rtrim($apiResult2, ")");
			$apiResult2 = json_decode($apiResult2, true);
			if( empty($apiResult2) || !empty($apiResult2["resultCode"]) ) 
			{
				return error(2, "获取房间信息失败(video_url error)");
			}
			$resultArr = array( "status" => $apiResult["isPlay"], "poster" => $apiResult["videoCoverUrl"], "hls_url" => $apiResult2["url"] );
			break;
		}
		$resultArr["type"] = $type;

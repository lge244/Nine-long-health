...
class index_EweiShopV2Page extends PluginMobilePage
{
    ...
    public function detail_new()
    {
        ...
        !empty($detail["tasktype"]) and !empty($detail["type"]) and $postfix = pdo_fetch("SELECT verb,unit FROM " . tablename("ewei_shop_task_list") . " WHERE id=:id and uniacid=:uniacid", array( ":uniacid" => $_W["uniacid"], ":id" => $detail["taskid"] ));
    ...
    public function picktask()
    {
        ...
        if( $_W["ispost"] ) 
        {
            ...
            empty($taskid) and show_json(0, "任务不存在");

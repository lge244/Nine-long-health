...
class TaskModel extends PluginModel 
{
	...
	protected function stoptime($task) 
	{
		...
		else if ($task['stop_type'] == 3) 
		{
			...
			{
				case '0': $stoptime = date('Y-m-d 00:00:00', strtotime('+1 day'));
				break;
				case '1': $stoptime = date('Y-m-d 00:00:00', strtotime('next Monday'));
				break;
				case '2': $stoptime = date('Y-m-d 00:00:00', mktime(0, 0, 0, date('n') + 1, 1, date('Y')));
				break;
			}
		}
		return $stoptime;
	...
	public function sentReward($recordid, $openid, $level = 0) 
	{
		...
		if (!(empty($rewards))) 
		{
			foreach ($rewards as $k => $reward ) 
			{
				...
				{
					case 'credit': m('member')->setCredit($openid, 'credit1', floatval($reward['reward_data']), array($_W['uid'], '任务中心奖励'));
					pdo_update('ewei_shop_task_reward', array('sent' => 1, 'get' => 1, 'gettime' => $time, 'senttime' => $time), array('id' => $reward['id']));
					break;
					case 'balance': m('member')->setCredit($openid, 'credit2', floatval($reward['reward_data']), array($_W['uid'], '任务中心奖励'));
					pdo_update('ewei_shop_task_reward', array('sent' => 1, 'get' => 1, 'gettime' => $time, 'senttime' => $time), array('id' => $reward['id']));
					break;
					case 'redpacket': pdo_update('ewei_shop_task_reward', array('get' => 1, 'gettime' => $time), array('id' => $reward['id']));
					break;
					case 'coupon': $data = array('uniacid' => $_W['uniacid'], 'merchid' => 0, 'openid' => $openid, 'couponid' => $reward['reward_data'], 'gettype' => 7, 'gettime' => time(), 'senduid' => $_W['uid']);
					pdo_insert('ewei_shop_coupon_data', $data);
					pdo_update('ewei_shop_task_reward', array('sent' => 1, 'get' => 1, 'gettime' => $time, 'senttime' => $time), array('id' => $reward['id']));
					break;
					case 'goods': pdo_update('ewei_shop_task_reward', array('get' => 1, 'gettime' => $time), array('id' => $reward['id']));
					break;
				}
			}
		}
	...
	public function posterProgressMessage($openid, $record) 
	{
		...
		$message = array( 'first' => array('value' => $record['niackname'] . '关注了您的海报，为您增加了1 点人气。', 'color' => '#ff0000'), 'keyword2' => array('title' => '业务状态', 'value' => $record['tasktitle'], 'color' => '#000000'), 'keyword3' => array('title' => '业务内容', 'value' => '海报任务', 'color' => '#000000'), 'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'), 'remark' => array('value' => '当前进度：' . $record['task_progress'] . '/' . $record['task_demand'] . "\n" . '扫描关注：' . $this_member['nickname'] . "\n" . '扫描时间：' . date('Y-m-d H:i') . '如有疑问请联系在线客服', 'color' => '#000000') );

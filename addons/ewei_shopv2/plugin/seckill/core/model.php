...
class SeckillModel extends PluginModel 
{
	...
	public function getTaskSeckillInfo() 
	{
		...
		$dbgoods = pdo_fetchall('select id,thumb,marketprice,thumb_url from ' . tablename('ewei_shop_goods') . ' where id in (' . implode(',', $goodsids) . ') and uniacid=' . $_W['uniacid'], array(), 'id');
		$goods = array();

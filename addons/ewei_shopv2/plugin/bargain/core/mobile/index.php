...
class Index_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function join() 
	{
		...
		if (!(empty($res['initiate']))) 
		{
			$count = pdo_get('ewei_shop_bargain_actor', array('goods_id' => $goods_id, 'openid' => $_W['openid'], 'status' => 0, 'orderid' => 0), 'id');
			if (!(empty($count['id']))) 
			{
	...
	private function sendBargainResult($openid, $cut_price, $now_price, $nickname, $iORr, $sORf, $last = 0) 
	{
		...
		if ($last == 1) 
		{
			$tag = 'bargain_fprice';
			$text = '砍到底价通知：' . "\n\n" . '[砍价人昵称]帮您砍到底价了，' . "\n" . '砍价结果：[砍掉或增加]了[砍价金额]元' . "\n" . '砍价时间：[砍价时间]' . "\n" . '当前成交价：[当前金额]元' . "\n\n" . '<a href=\'' . $url . '\'>点击查看详情</a>';
			$message = array( 'first' => array('value' => '砍到底价通知' . "\n", 'color' => '#000000'), 'keyword1' => array('title' => '任务名称', 'value' => $nickname . '帮你砍到底价', 'color' => '#000000'), 'keyword2' => array('title' => '通知类型', 'value' => '砍到底价通知', 'color' => '#000000'), 'remark' => array('value' => '砍价金额：' . $iORr . '了' . $cut_price . '元' . "\n" . '砍价时间：' . $time . "\n" . '当前价格：' . $now_price . '元' . "\n\n" . '点击立即下单', 'color' => '#000000') );
		}
		else 
		{
			$tag = 'bargain_message';

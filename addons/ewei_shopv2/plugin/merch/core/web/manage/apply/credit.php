...
class Credit_EweiShopV2Page extends MerchWebPage
{
    protected function applyData($status, $st)
    {
        global $_W;
        global $_GPC;
        empty($status) and $merchid = $_W["merchid"];
    ...
    protected function post()
    {
        ...
        if( $set["applycashalipay"] == 1 ) 
        {
            ...
            if( !empty($last_data) && $last_data["applytype"] != 2 ) 
            {
        ...
        if( $set["applycashcard"] == 1 ) 
        {
            ...
            if( !empty($last_data) && $last_data["applytype"] != 3 ) 
            {
        ...
        if( !empty($last_data) && array_key_exists($last_data["applytype"], $type_array) ) 
        {

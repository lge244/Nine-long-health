...
class MerchModel extends PluginModel 
{
	...
	{
		return array( 'text' => m('plugin')->getName('creditshop'), 'goods' => array( 'text' => '商品', 'main' => '查看列表', 'view' => '查看详细', 'add' => '添加-log', 'edit' => '修改-log', 'delete' => '删除-log', 'xxx' => array('property' => 'edit') ), 'log' => array('text' => '订单/记录', 'exchange' => '兑换记录', 'draw' => '抽奖记录', 'order' => '待发货', 'convey' => '待收货', 'finish' => '已完成', 'verifying' => '待核销', 'verifyover' => '已核销', 'verify' => '全部核销', 'detail' => '详情', 'doexchange' => '确认兑换-log', 'export' => '导出明细-log'), 'comment' => array('text' => '评价管理', 'edit' => '回复评价', 'check' => '审核评价') );
	}
	...
	{
		return array( 'text' => m('plugin')->getName('taobao'), 'main' => '获取宝贝', 'jingdong' => array('text' => '京东助手', 'main' => '获取宝贝'), 'one688' => array('text' => '1688宝贝助手', 'main' => '获取宝贝'), 'taobaocsv' => array('text' => '淘宝CSV助手', 'main' => '获取宝贝') );
	}
	...
	public function sendMessage($sendData, $message_type) 
	{
		...
		if (($message_type == 'merch_apply') && empty($usernotice['merch_apply'])) 
		{
			if ($tm['is_advanced']) 
			{
				...
				$tm['msguser'] = 0;
				$data = array( array('name' => '商户名称', 'value' => $sendData['merchname']), array('name' => '主营项目', 'value' => $sendData['salecate']), array('name' => '联系人', 'value' => $sendData['realname']), array('name' => '手机号', 'value' => $sendData['mobile']), array('name' => '申请时间', 'value' => date('Y-m-d H:i:s', $sendData['applytime'])) );
				$tag = 'merch_apply';
				$text = '[商户名称]在[申请时间]提交了入驻申请，请到后台查看~';
				$time = date('Y-m-d H:i:s', time());
				$message = array( 'first' => array('value' => $sendData['merchname'] . '入驻申请', 'color' => '#ff0000'), 'keyword2' => array('title' => '业务状态', 'value' => (!(empty($tm['merch_applytitle'])) ? $tm['merch_applytitle'] : '商户入驻申请'), 'color' => '#000000'), 'keyword3' => array('title' => '业务内容', 'value' => (!(empty($tm['merch_apply'])) ? $tm['merch_apply'] : '[商户名称]在[申请时间]提交了入驻申请。'), 'color' => '#000000'), 'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'), 'remark' => array('value' => "\n" . '请到后台查看~', 'color' => '#000000') );
				if (!(empty($openid))) 
				{
					foreach ($openid as $v ) 
					{
		...
		if (($message_type == 'merch_apply_money') && empty($usernotice['merch_apply_money'])) 
		{
			if ($tm['is_advanced']) 
			{
				...
				$tm['msguser'] = 1;
				$data = array( array('name' => '商户名称', 'value' => $sendData['merchname']), array('name' => '金额', 'value' => $sendData['salecate']), array('name' => '联系人', 'value' => $sendData['realname']), array('name' => '手机号', 'value' => $sendData['mobile']), array('name' => '申请时间', 'value' => date('Y-m-d H:i:s', $sendData['applytime'])) );
				$tag = 'merch_applymoney';
				$text = '[商户名称]在[申请时间]提交了提现申请,提现金额' . $sendData['money'] . '.[联系人] [手机号].';
				$time = date('Y-m-d H:i:s', time());
				$message = array( 'first' => array('value' => $sendData['merchname'] . '入驻申请', 'color' => '#ff0000'), 'keyword2' => array('title' => '业务状态', 'value' => (!(empty($tm['merch_applymoneytitle'])) ? $tm['merch_applymoneytitle'] : '商户提现申请'), 'color' => '#000000'), 'keyword3' => array('title' => '业务内容', 'value' => (!(empty($tm['merch_apply'])) ? $tm['merch_apply'] : '[商户名称]在[申请时间]提交了提现申请,提现金额' . $sendData['money'] . '.[联系人] [手机号].'), 'color' => '#000000'), 'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'), 'remark' => array('value' => "\n" . '请到后台查看~', 'color' => '#000000') );
				if (!(empty($openid))) 
				{
					foreach ($openid as $v ) 
					{
			...
			if (!(empty($order))) 
			{
				...
				foreach ($order as $k => $v ) 
				{
					...
					if (($iscredit['iscreditmoney'] != 1) && ($list['creditrate'] != 0)) 
					{
						...
						{
							credit}
						+= floor(floatval($v['credit'] / $list['creditrate']));
					}
				}
			}
			$list['orderids'] = $orderids;

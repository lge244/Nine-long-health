...
class GroupsModel extends PluginModel 
{
		...
		if( $delRefund == true ) 
		{
			...
			else 
			{
				...
				else 
				{
					if( $order_refund["refundstatus"] == 1 ) 
					{
						$msg = array( "first" => array( "value" => "您的订单已经完成退款！", "color" => "#4a5077" ), "keyword1" => array( "title" => "退款金额", "value" => "¥" . $applyprice . "元", "color" => "#4a5077" ), "keyword2" => array( "title" => "商品详情", "value" => $goods2, "color" => "#4a5077" ), "keyword3" => array( "title" => "订单编号", "value" => $order["orderno"], "color" => "#4a5077" ), "remark" => array( "value" => "退款金额 ¥" . $applyprice . (string) $refundtype . "\r\n 期待您再次购物！", "color" => "#4a5077" ) );
						$this->sendGroupsNotice(array( "openid" => $openid, "tag" => "groups_refund", "default" => $msg, "datas" => $datas ));
					}
				}
			}
		...
		else 
		{
			if( $order["status"] == 1 ) 
			{
				if( $order["success"] == 1 ) 
				{
					...
					$msgteam = array( "first" => array( "value" => "拼团已经成功组团！", "color" => "#4a5077" ), "keyword1" => array( "title" => "待办项目", "value" => $goods, "color" => "#4a5077" ), "keyword2" => array( "title" => "待办环节", "value" => "等待发货", "color" => "#4a5077" ), "keyword3" => array( "title" => "更新时间", "value" => date("Y-m-d H:i", time()), "color" => "#4a5077" ), "remark" => array( "value" => $remarkteam, "color" => "#4a5077" ) );
					if( !empty($tm["openid"]) ) 
					{
						$openids = explode(",", $tm["openid"]);
						foreach( $openids as $value ) 
	...
	public function allow($orderid, $times = 0, $verifycode = "", $openid = "") 
	{
		...
		if( !empty($order["is_team"]) && ($order["status"] <= 0 || $order["success"] <= 0) ) 
		{
		...
		{
			if( $order["dispatchtype"] == 1 && 0 < $order["refundid"] && 0 < $order["refundstate"] ) 
			{

...
foreach( $sets as $key => $value ) 
{
	...
	$times = $hours * 60 * 60;
	$orders = pdo_fetchall("select id,orderno,openid,credit,creditmoney,price,freight,status,pay_type,teamid,apppay,isborrow,borrowopenid,more_spec from " . tablename("ewei_shop_groups_order") . "\r\n            where  uniacid=" . $_W["uniacid"] . " and status = 1 and pay_type !='other' and success = -1 and refundtime = 0 and canceltime + " . $times . " <= " . time() . " order by id desc ");

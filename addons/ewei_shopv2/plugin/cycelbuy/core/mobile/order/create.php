...
class Create_EweiShopV2Page extends PluginMobilePage 
{
	...
	public function caculate() 
	{
		...
		if( is_array($goodsarr) ) 
		{
			...
			foreach( $goodsarr as &$g ) 
			{
				...
				if( !empty($optionid) ) 
				{
					...
					if( !empty($option) ) 
					{
						...
						$cycelbuy_periodic = explode(",", $option["cycelbuy_periodic"]);
						list($cycelbuy_day, , $cycelbuy_num) = $cycelbuy_periodic;
					}
				}
				$data["isdiscount"] = 0;
				$data["isnodiscount"] = 1;
			...
			if( empty($dflag) && empty($saleset["dispatchnodeduct"]) ) 
			{
				$deductprice2 += $dispatch_price;
			}
	...
	public function submit() 
	{
		...
		if( $open_redis ) 
		{
			$redis_key = (string) $_W["uniacid"] . "_order_submit_" . $openid;
			$redis = redis();
			if( !is_error($redis) ) 
			{
			...
			if( !empty($optionid) ) 
			{
				...
				if( !empty($option) ) 
				{
					...
					if( $stock_num != -1 ) 
					{
						...
						if( !empty($data["unite_total"]) && $stock_num - intval($total_array[$goodsid]["total"]) < 0 ) 
						{
				...
				list($cycelbuy_day, , $cycelbuy_num) = $cycelbuy_periodics;
			}
		...
		if( $diyform_plugin && is_array($_GPC["diydata"]) && !empty($order_formInfo) ) 
		{

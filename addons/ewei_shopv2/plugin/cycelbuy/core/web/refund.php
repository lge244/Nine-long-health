...
class Refund_EweiShopV2Page extends PluginWebPage
{
	...
	public function detail()
	{
		...
		list(, , $order['num']) = explode(',', $order['cycelbuy_periodic']);
	...
	public function save()
	{
		...
		if ($result !== false) {
			if ($type == 1) {
				if ($data['is_all'] == 0) {
					$address = pdo_fetchall('select * from ' . tablename('ewei_shop_cycelbuy_periods') . ' where orderid=:orderid and status = 0 order by id asc limit 1', array(':orderid' => $orderid));
				}
				else {

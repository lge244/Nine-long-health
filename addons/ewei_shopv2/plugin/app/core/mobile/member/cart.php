...
class Cart_EweiShopV2Page extends AppMobilePage
{
    ...
    public function add()
    {
        ...
        $goods = pdo_fetch("select id,marketprice,`type`,total,diyformid,diyformtype,diyfields, isverify,merchid,hasoption from " . tablename("ewei_shop_goods") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
        if( empty($goods) ) 
        {
    ...
    public function update()
    {
        ...
        $optionid = intval($_GPC["optionid"]);
        empty($goodstotal) and $data = pdo_fetch("select * from " . tablename("ewei_shop_member_cart") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1 ", array( ":id" => $id, ":uniacid" => $_W["uniacid"], ":openid" => $_W["openid"] ));
        if( empty($data) ) 
        {

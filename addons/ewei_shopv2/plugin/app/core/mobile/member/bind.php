...
class Bind_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		app_json(array('member' => $memberArr, 'binded' => !empty($member['mobile']) && !empty($member['mobileverify']) ? 1 : 0, 'endtime' => $endtime, 'smsimgcode' => $wapset['smsimgcode'], 'verifycode_img' => $verifycode_img));
	}
	...
	public function imageChange()
	{
		...
		$domain = 'https://' . $_SERVER['SERVER_NAME'];
		$verifycode_img = $domain . '/app/ewei_shopv2_api.php?i=' . $_W['uniacid'] . '&r=sms.captcha&time=' . time() . '&openid=' . $_W['openid'];
		app_json(array('verifycode_img' => $verifycode_img));
	}
}

...
class Index_EweiShopV2Page extends Base_EweiShopV2Page 
{
	public function register() 
	{
		...
		if( $_W["ispost"] ) 
		{
			...
			if( $template_flag == 1 ) 
			{
				...
				if( !empty($member["uid"]) && !empty($mc_data) ) 
				{
		...
		if( $to_check_heads && empty($member["isheads"]) ) 
		{
		...
		$result = array( "set" => array( "open" => $set["open"], "texts" => $this->set["texts"], "regbg" => (empty($set["regbg"]) ? $_W["siteroot"] . "addons/ewei_shopv2/plugin/dividend/template/mobile/default/static/images/banner.jpg" : $set["regbg"]), "condition" => (int) $set["condition"], "open_protocol" => $set["open_protocol"], "applytitle" => $set["applytitle"], "applycontent" => $set["applycontent"] ), "member" => array( "headsblack" => (int) $member["headsblack"], "isheads" => (int) $member["isheads"], "headsstatus" => (int) $member["headsstatus"], "realname" => $member["realname"], "mobile" => $member["mobile"] ), "status" => intval($status), "member_totalcount" => $member_totalcount, "member_count" => $member_count, "commissiondownline_totalcount" => $commissiondownline_totalcount, "commissiondownline_count" => $commissiondownline_count, "total_commission_totalcount" => $total_commission_totalcount, "total_commission_count" => $total_commission_count, "cash_commission_totalcount" => $cash_commission_totalcount, "cash_commission_count" => $cash_commission_count, "template_flag" => $template_flag, "f_data" => $appDatas["f_data"], "fields" => $appDatas["fields"] );
		app_json($result);
	}
	public function main() 
	{
		...
		$member["applycount"] = pdo_fetchcolumn("select count(id) from " . tablename("ewei_shop_dividend_apply") . " where mid=:mid and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":mid" => $member["id"] ));
		$result = array( "member" => $member, "isbuild" => $isbuild );
		app_json($result);
	}

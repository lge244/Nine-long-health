...
class Pay_EweiShopV2Page extends AppMobilePage 
{
	public function main() 
	{
		...
		{
			app_error(1, "该活动已失效，请浏览其他商品或联系商家！(1)");
		}
		if( empty($order["gstatus"]) || !empty($order["gdeleted"]) ) 
		{
			app_error(1, $order["title"] . "已下架!");
		}
		if( !empty($order["more_spec"]) ) 
		{
			if( $order["og_stock"] <= 0 ) 
			{
				app_error(1, $order["og_title"] . "库存不足(2)!");
			}
		}
		else 
		{
			if( $order["stock"] <= 0 ) 
			{
				app_error(1, $order["title"] . "库存不足(1)!");
			}
		}
		if( !empty($teamid) ) 
		{
			...
			foreach( $team_orders as $key => $value ) 
			{
				...
				{
					app_error(1, "该活动已过期，请浏览其他商品或联系商家！(2)");
				}
				if( $team_orders && $value["success"] == 1 ) 
				{
					app_error(1, "该活动已结束，请浏览其他商品或联系商家！(3)");
				}
			}
		...
		if( $order["status"] == -1 ) 
		{
			app_error(1, "该活动已过期，请浏览其他商品或联系商家！(4)");
		}
		else 
		{
			if( 1 <= $order["status"] ) 

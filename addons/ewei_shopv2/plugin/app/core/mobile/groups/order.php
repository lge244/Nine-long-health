...
class Order_EweiShopV2Page extends AppMobilePage 
{
	...
	public function details() 
	{
		...
		switch( $order["status"] ) 
		{
			...
			break;
			case "1": $order["status_str"] = "买家已付款";
			break;
			case "2": $order["status_str"] = "卖家已发货";
			break;
			case "3": $order["status_str"] = "交易完成";
			break;
		}
		...
		app_json(array( "express" => $express, "goodRefund" => $goodRefund, "order" => $order, "address" => $address, "store" => $stores, "verifytotal" => $verifytotal, "verifynum" => $verifynum, "carrier" => $carrier, "verify" => $verify, "goods" => $goods ));
	}
	public function create_order() 
	{
		...
		if( !empty($set["discount"]) && $heads == 1 ) 
		{
			...
			{
				if( !empty($goods["headstype"]) && 0 < $goods["headsdiscount"] ) 
				{
		...
		if( $_W["ispost"] == true ) 
		{
			...
			if( $isverify && (empty($_GPC["realname"]) || empty($_GPC["mobile"])) ) 
			{
		...
		$data = array( "is_verify" => ($isverify ? 1 : 0), "is_ladder" => $is_ladder, "is_more_spec" => $goods["more_spec"], "type" => $type, "teamid" => $teamid, "heads" => $heads, "headsmoney" => $goods["headsmoney"], "credit" => $credit, "ladder" => $ladder, "goods" => $new_goods, "stores" => $stores, "address" => $address, "freight" => $goods["freight"], "price" => round(floatval($price + $goods["freight"]), 2) );
	...
	public function express() 
	{
		...
		app_json(array( "com" => $order["expresscom"], "sn" => $order["expresssn"], "status" => $status, "count" => count($goods), "thumb" => tomedia($goods["thumb"]), "expresslist" => $expresslist ));
	}
}
?>
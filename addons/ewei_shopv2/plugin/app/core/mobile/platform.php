...
class Platform_EweiShopV2Page extends AppMobilePage
{
    ...
    private function verifySign()
    {
        ...
        if( empty($time) ) 
        {
            app_error(AppError::$ParamsError, "参数错误(time)");
        }

        if( $time + 60 < time() ) 
        {
            app_error(AppError::$ParamsError, "sign失效");
        }

        $sign = trim($_GPC["sign"]);
        if( empty($time) ) 
        {
            app_error(AppError::$ParamsError, "参数错误(sign)");
        }
        ...
        if( empty($site_id) ) 
        {
            app_error(AppError::$ParamsError, "参数错误(site_id)");
        }

...
class Apply_EweiShopV2Page extends DividendWebPage 
{
	public function main() 
	{
		...
		else if (!(cv('dividend.apply.view' . $status))) 
		{
			$this->message('你没有相应的权限查看');
		}
		...
		if ($_GPC['export'] == '1') 
		{
			...
			foreach ($list as &$row ) 
			{
				...
				foreach ($orders as &$order ) 
				{
					...
					else if ($order['dividend_status'] == -1) 
					{
						$order['dstatus'] = '未通过';
					}
					$row['goods'] = $goods;
	...
	public function pay($params = array(), $mine = array()) 
	{
		...
		if ($apply['type'] < 2) 
		{
			...
			{
				$result = m('finance')->payRedPack($member['openid'], $pay, $apply['applyno'], $apply, $set['texts']['dividend'] . '打款', $data['paytype']);
			...
			{
				$result = m('finance')->pay($member['openid'], $apply['type'], $pay, $apply['applyno'], $set['texts']['dividend'] . '打款');
			}
			if (is_error($result)) 
			{
